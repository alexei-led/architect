#!/usr/bin/env python3
"""Summarize the latest agent-skills-eval run for local feedback."""
...
DEFAULT_WORKSPACE = Path("/tmp/architect-skill-eval-workspace")
...
def collect(iteration: Path) -> tuple[dict[str, Counts], dict[str, SkillStats], list[Failure]]:
...
def render_text(
    ...
    baseline_failures = [failure for failure in failures if failure.mode == "without_skill"]
    lines = [
        "SKILL EVAL SUMMARY",
        "------------------",
...
def render_markdown(
    ...
    lines.extend(
        [
            ...
            "## With-skill failures to fix",
            "",
            "Fix the skill, or tighten the eval only when judge evidence shows the "
            "assertion is wrong.",
            "",
            *markdown_failure_lines(with_failures, limit),
            ...
            "## Baseline failures",
            "",
            "Baseline failures show skill lift. They are not failures to fix unless "
            "baseline beats the skill or the eval is malformed.",
            "",
